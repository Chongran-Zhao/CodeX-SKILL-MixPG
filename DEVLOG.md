...
- 2026-05-19：继续用 traction case 比较 `master` 与 `codex/modularize-viscoelasticity-model-to-hpp-file` 时，补充 compare-mode/driver 失败判定规则：一是如果两个 branch 在同一个共享 case 上同阶段共同发散或 `MPI_Abort`，应先判定为共享 case 不稳定或共享运行失败，而不是“比较成功”；二是如果 driver 通过 `mpirun ... | tee driver_log.txt` 运行，必须保留 `mpirun` 的真实退出状态，例如使用 `set -o pipefail`，并且即使 shell 表面退出码为 `0`，只要日志里出现 `MPI_Abort`、`local_NR_iteration solver is diverging` 等致命标记，也必须按 driver 失败处理。
- 2026-05-19：在更温和的 traction branch-compare baseline 中，两个分支都跑通并得到完全一致的 `Force_disp_record.txt`；据此补充 branch-compare 报告规则，要求 compare mode 使用专门的 branch-compare report variant，包含分支名、commit hash、共享 case、workflow/artifact/result 对比和叠加曲线。同时补充两个后处理护栏：`post_surface_force` 必须显式传当前 run 的 `-time_end`，不能依赖默认值；compare 报告前必须分别检查两个 build 目录的 `paras_pos_vis.yml.time_end` 是否超过实际可用输出范围。
